...
不再需要独立设置模块。模板内的公共规则引用 `https://github.com/NSNanoCat/PreferencePanes/releases/latest/download/api.js`，处理模块页面、渲染资源和 POST /api/get、set、delete。请求使用 form 完整 @root.path，不鉴权；设置字段仍从当前 Enhanced 版本的 JSON 取得。首次迁移须先发布 PreferencePanes 的新 api.js，再更新 Enhanced 模块。

dev 工作流用一次 Gist API 更新业务脚本、JSON、配置响应和订阅，并删除旧 settings.dev.bundle.js。正式版文件由 Release 工作流一起上传，URL 固定到对应 tag。dev 按现有业务脚本惯例滚动更新；通用 API 独立升级，不和业务配置版本绑定。
